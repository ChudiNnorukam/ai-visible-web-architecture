...
| `chudi.dev` publishes explicit AI-crawler access policy. | `https://chudi.dev/robots.txt` | [architecture/the-fifteen-check-audit.md](../architecture/the-fifteen-check-audit.md) | `robots.txt AI-crawler allow-list` |
| The portfolio publishes cross-property entity discovery. | `https://chudi.dev/entitymap.json`, `https://citability.dev/entitymap.html` | [architecture/portfolio-repository-map.md](../architecture/portfolio-repository-map.md) | endpoint presence verified manually; automated check planned |
| citability.dev publishes a structured agent-action surface. | `https://citability.dev/.well-known/agent-actions` | [architecture/agent-readiness-layer.md](../architecture/agent-readiness-layer.md) | endpoint presence verified manually; schema validation planned |

## Target state claims (planned audit checks)

These claims belong to the chapters added in the 2026-05-01 evolution
(AVR audit, entity authority, backlinks and off-page, agent
